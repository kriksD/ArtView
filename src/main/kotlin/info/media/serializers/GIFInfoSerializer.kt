...
class GIFInfoSerializer : KSerializer<GIFInfo> {
    override val descriptor: SerialDescriptor = buildClassSerialDescriptor("GIFInfo") {
        element<String>("type")
        element<Int>("id")
        element<String>("path")
        element<String>("name")
    ...
    override fun deserialize(decoder: Decoder): GIFInfo = decoder.decodeStructure(descriptor) {
        var id: Int? = null
        ...
        return@decodeStructure if (MediaInfoFixer.isMediaInfoBroken(info)) {
            MediaInfoFixer.fixMediaInfo(info) as GIFInfo
        } else {
            info
        }
    }
    ...
    @OptIn(ExperimentalSerializationApi::class)
    override fun serialize(encoder: Encoder, value: GIFInfo) = encoder.encodeStructure(descriptor) {
        encodeStringElement(descriptor, 0, MediaType.GIF.toString())
